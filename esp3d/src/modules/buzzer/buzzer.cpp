...
void process() {
  if (esp3d_buzzer.started()) {
    ...
    if (current) {
      tone(ESP3D_BUZZER_PIN, (unsigned int)current->frequency,
...
void BuzzerDevice::no_tone() { noTone(ESP3D_BUZZER_PIN); }

bool BuzzerDevice::isPlaying() { return !(_head == nullptr); }
